...
describe("The User Api /api/users", () =>{
  ...
  describe("GET /api/users/:username", () => {
    test("returns the users profile and list of grids", async () =>{
      const res = await request('http://localhost:3000')
        .get('/api/users/JohnDough20/')
        .expect(200)
        .expect('Content-Type', /json/)
      expect(res.body.email).toBe('JohnDough20@test.com')
      ...
      expect(res.body.grids.length).toBeGreaterThan(0)
      expect(res.body.grids[0].events[0].eventName).toBe('My Starter Grid')
    })
  })
})
